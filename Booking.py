...
class Booking:
    ...
    def getInfo(self, idNumber):
        ...
        info = c.fetchone()
        print("\n\tYOUR INFO\nCustomer: %s\nPeople: %s\nDate: %s\nTime: %s" % (
            str(info[1]), str(info[2]), str(info[3]), str(info[4])))
        conn.close()
    ...
    def getInfoBrief(self, idNumber):
        ...
        info = c.fetchone()
        print("People: %s\tDate: %s\tTime: %s\tCount: %s" % (str(info[1]), str(info[3]), str(info[4]), str(info[2])))
        conn.close()
    ...
    def saveToDB(self):
        conn = sqlite3.connect("database.db")
        c = conn.cursor()
        c.execute('insert into dates(owner,howmanypeople,dateof,timeof) values(?,?,?,?)',
                  [self.owner, self.numberOfPeople, self.dateOf, self.timeOf])
        conn.commit()
        return c.lastrowid
        conn.close()
...
def newBooking():
    ...
    ownerRaw = str(input("customer name(Raj, Gowtham etc.): "))
